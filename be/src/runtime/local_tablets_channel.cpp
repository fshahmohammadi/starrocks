...
Status LocalTabletsChannel::_open_all_writers(const PTabletWriterOpenRequest& params) {
    ...
    // Allocate a Slice in _mem_pool from a proto string word.
    ...
    // Build dict from repeated proto string words. Codes are 1-based:
    // passthrough source dicts require this to match the scan node's IDictManager convention;
    // validation dicts don't care about code values, so 1-based is fine for both.
    ...
    for (auto& slot : params.schema().slot_descs()) {
        if (slot.global_dict_words_size()) {
            ...
            for (int i = 0; i < slot.global_dict_words_size(); i++) {
                ...
                dict_map.emplace(slice, static_cast<int32_t>(i + 1));
            }
            GlobalDictsWithVersion<GlobalDictMap> dict;
            dict.dict = std::move(dict_map);
        ...
        if (slot.passthrough_source_dict_words_size()) {
            // Build 1-based vector: index 0 is unused placeholder, source_dict[code] = Slice.
            std::vector<Slice> source_dict;
            source_dict.reserve(slot.passthrough_source_dict_words_size() + 1);
            source_dict.emplace_back(); // index 0 placeholder
            for (int i = 0; i < slot.passthrough_source_dict_words_size(); i++) {
                ASSIGN_OR_RETURN(auto slice, alloc_slice(slot.passthrough_source_dict_words(i)));
                source_dict.emplace_back(slice);
            }
            _passthrough_source_dicts.emplace(std::string(slot.col_name()), std::move(source_dict));
        }
    }
